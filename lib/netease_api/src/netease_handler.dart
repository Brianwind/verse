...
void neteaseInterceptor(
  ...
  if (option.method == 'POST' &&
    ...
    option.extra['cookiesHash'] =
        await loadCookiesHash(cookies: cookies) +
...
void _handleWeApi(RequestOptions option) {
  ...
  final encrypted3 = encrypter3.encryptBytes(
...
void _handleEApi(RequestOptions option, List<Cookie> cookies) {
  ...
  var header = {};
  Map<String, String> cookiesMap = cookies.fold(<String, String>{}, (
    map,
    element,
  ) {
  ...
  var digest =
      Encrypted(
        MD5Digest().process(Uint8List.fromList(utf8.encode(message))),
      ).base16;
  var data = '$url-36cd479b6b5-$body-36cd479b6b5-$digest';

  final encrypted =
      Encrypter(
        AES(Key.fromUtf8(_KeyEApi), mode: AESMode.ecb),
      ).encrypt(data, iv: IV.fromLength(0)).base16.toUpperCase();
